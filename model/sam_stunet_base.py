...
class SAM(nn.Module):
    def __init__(self, dim, ca_num_heads=4, qkv_bias=False, qk_scale=None, 
        ...
        # self.gru_v = nn.GRU(input_size=dim, hidden_size=dim, num_layers=1, \
        #                   bias=True, batch_first=True, dropout=0.0, \
        #                   bidirectional=False)
    ...
    def forward(self, x):
        ...
        B, N, C = x.shape
        v = self.v(x)
        s = self.gru_s(x)[0].reshape(B, N, self.ca_num_heads, C//self.ca_num_heads).permute(2, 0, 3, 1)
    ...
    def __init__(
        ...
        for level, mult in enumerate(channel_mult):
            for _ in range(num_res_blocks):
                ...
                layers.append(SAM(dim=out_ch, ca_num_heads=num_heads_upsample, expand_ratio=2))
...
#                     ch = int(mult) * model_channels
                ...
                layers.append(SAM(dim=ch*2, ca_num_heads=num_heads_upsample, expand_ratio=2))
...
#         x = x.reshape(x.size(0), -1, x.size(2))
        ...
        emb_text = self.emb_text(enc_text_clip, force_mask=force_mask)
        emb += emb_text
