...
def main():
    ...
    assert args.guidance_param == 5.5, "Experiment requires guidance_param=3.5"
    ...
    for data_name, data in data_dict.items():
        with open(f"{out_path}/{data_name}.skels", "w") as skel_file:
            with open(f"{out_path}/{data_name}.text", "w") as text_file:
                ...
                transform_tmp_2_slt_data(f"{out_path}/{data_name}", data_dim=model.in_channels, des=0, out_name=f"{data_name}_back_translation")
...
# --model_path /kaggle/working/mdm/save/OURs/model000050000.pt \
# --output_dir /kaggle/working/mdm/save/OURs/50k \
# --guidance_param 5.5 \
# --batch_size 64